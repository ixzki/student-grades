...
- **管理员端**：
  ...
  - 学生管理：查看/新增/删除学生、编辑班级、导出学生名单（CSV）、重置学生密码（生成新的 6 位随机数字密码）
  - 考试列表：考试详情（查看本次考试所有学生成绩表）、班级排名开关（控制学生是否能查看排名）
  - 修改管理员密码、退出登录
- **学生端**：
  - 登录：**姓名 + 密码**
...
- 管理员：仍使用 `username`（默认 `admin`）登录
- 学生：使用 **姓名（User.name）+ 密码** 登录

> 为简化实现，目前系统默认要求“学生姓名唯一”。

## CSV 导入格式（管理员后台）

- 前两列表头必须为：`姓名,班级`
- 后续列名会自动识别为科目名（动态）
- 若学生不存在：系统会自动创建学生，并生成 **6 位随机数字密码**（可在学生列表中查看/导出，或随时重置）。
...
- `GET/PATCH /api/admin/exams/[examId]`：考试详情（含成绩表）/ 更新班级排名开关
- `POST /api/admin/import-grades`：管理员上传 CSV 导入成绩
- `GET/POST/PATCH/DELETE /api/admin/students`：学生列表、创建、编辑班级、重置密码、删除
